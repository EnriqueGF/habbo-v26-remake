...
switch($mode){
	case 1:
		$str = "friends";
		$get_em = mysql_query("SELECT friendid FROM messenger_friendships WHERE userid = '".$my_id."' OR friendid = '".$my_id."' ORDER BY friendid LIMIT 500") or die(mysql_error());
		break;
	case 2:
		$str = "groups";
	...
	if($results > 0){
		...
		$num = 0;
			...
			while($row = mysql_fetch_assoc($get_em)){
				...
				if($row['is_current'] == 1){ echo "<div class=\"favourite-group\" title=\"Favorito\"></div>\n"; }
				if($row['member_rank'] > 1 && $groupdata['ownerid'] !== $my_id){ echo "<div class=\"admin-group\" title=\"Admin\"></div>\n"; }
				if($groupdata['ownerid'] == $my_id && $row['member_rank'] > 1){ echo "<div class=\"owned-group\" title=\"Propietario\"></div>\n"; }
		...
		echo "Modo no v&aacute;lido";
		}
	} else {
		echo "<ul id=\"quickmenu-" . $str . "\">\n	<li class=\"odd\">No tienes ning&uacute;n " . $str . "</li>\n</ul>";
	}

	if($mode == "3"){
	echo "<p class=\"create-room\"><a href=\"client.php?shortcut=roomomatic\" onclick=\"HabboClient.openShortcut(this, 'roomomatic'); return false;\" target=\"client\">Crear una sala</a></p>";
	} elseif($mode == "2"){
