...
$refer = $_SERVER['HTTP_REFERER']; $pos = strpos($refer, "register.php"); if($pos == false) { exit; }
...
$failure = false;
    ...
    // Name validation
    if($tmp > 0){
        $error['name'] = "Ese nombre de usuario ya est&aacute; cogido. Elige otro.";
        $failure = true;
    } elseif($filter !== $name){
        $error['name'] = "El nombre de usuario contiene caracteres no v&aacute;lidos.";
        $failure = true;
    } elseif(strlen($name) > 24){
        $error['name'] = "El nombre de usuario es demasiado largo.";
        $failure = true;
    } elseif(strlen($name) < 1){
        $error['name'] = "Elige un nombre de usuario para continuar.";
        $failure = true;
    }
...
// MOD- Names validation
    ...
    if ($pos == true) {
        ...
        $error['name'] = "No tienes permiso para usar ese prefijo.";
    ...
    if($password !== $retypedpassword){
        $error['password'] = "Las contrase&ntilde;as no coinciden.";
        $failure = true;
    } elseif(strlen($password) < 6){
        $error['password'] = "La contrase&ntilde;a es demasiado corta.";
        $failure = true;
    ...
    // E-Mail validation
    if(strlen($email) < 6){
        $error['mail'] = "Introduce un e-mail v&aacute;lido.";
        $failure = true;
    } elseif($email_check !== 1){
        $error['mail'] = "Debes introducir una direcci&oacute;n de e-mail.";
        $failure = true;
    } elseif($email !== $retypedemail){
        $error['mail'] = "Los e-mails no coinciden.";
        $failure = true;
    }

    // Date of birth validation
    if($day < 1 || $day > 31 || $month > 12 || $month < 1 || $year < 1920 || $year > 2008){
        $error['dob'] = "Introduce una fecha de nacimiento correcta.";
        $failure = true;
    }
    ...
    if($accept_tos !== "true"){
        $error['tos'] = "Lee y acepta las condiciones para registrarte.";
        $failure = true;
    }

    // Try to (we really can't properly) validate figure
    // The Shockwave avatar picker on the register page frequently fails to set
    // bean_figure; fall back to a default look so registration still works.
    if(empty($figure)){
        $figure = "hr-115-42.hd-190-10.ch-215-66.lg-285-77.sh-290-80";
    }

    // Check gender (default to M without blocking registration)
    ...
    if($failure == false){
        ...
        mysql_query("INSERT INTO users (name,password,email,birth,figure,sex,rank,hbirth,ipaddress_last,postcount,tickets,credits,lastvisit,screen,rea) VALUES ('".$name."','".$password."','".$email."','".$dob."','".$figure."','".$gender."','1','".$date_normal."','".$remote_ip."','0','0','".$scredits."','".$date_full."','wide','enabled')") or die(mysql_error());
        ...
        if($scredits > 0){
            mysql_query("INSERT INTO cms_transactions (userid,date,amount,descr) VALUES ('".$userid."','".$date_full."','".$scredits."','Bienvenido a " . $sitename . "!')") or die(mysql_error());
        }

        $_SESSION['username'] = $name;
        $_SESSION['password'] = $password;

        header("Location: security_check.php?me");
        ...
        <div id="register-column-left">
            ...
            <div id="register-avatar-editor">
                ...
                <div class="register-avatars clearfix">
                    <div class="register-avatar" style="background-image: url(/habbo-imaging/avatar/hr-505-39.hd-626-1.ch-691-83.lg-705-67.sh-735-91.ha-1001-.ea-1401-84,s-0.g-1.d-4.h-4.a-0,b72abee33635a00982228c1f9215ae9a.gif)">
                    ...
                    <div class="register-avatar" style="background-image: url(/habbo-imaging/avatar/hr-530-44.hd-615-10.ch-630-89.lg-705-67.sh-740-62.fa-1206-91.wa-2001-,s-0.g-1.d-4.h-4.a-0,161dda9150b648cd14ff1aa9d9dcc5ad.gif)">
                        <input type="radio" name="randomFigure" value="F-hr-530-44.hd-615-10.ch-630-89.lg-705-67.sh-740-62.fa-1206-91.wa-2001-" />
                    </div>
                    <div class="register-avatar" style="background-image: url(/habbo-imaging/avatar/hr-681-40.hd-629-1.ch-675-86.lg-700-78.sh-725-82,s-0.g-1.d-4.h-4.a-0,1aba810a364a5410115f463f6963f7e1.gif)">
                        <input type="radio" name="randomFigure" value="F-hr-681-40.hd-629-1.ch-675-86.lg-700-78.sh-725-82" />
                    </div>
                </div>
                <h3>Chico</h3>
                <div class="register-avatars clearfix">
                    <div class="register-avatar" style="background-image: url(/habbo-imaging/avatar/hr-105-34.hd-205-6.ch-878-78.lg-281-76.sh-906-67,s-0.g-1.d-4.h-4.a-0,469e46ac01e81f244eb50b4ca7a43520.gif)">
                        <input type="radio" name="randomFigure" value="M-hr-105-34.hd-205-6.ch-878-78.lg-281-76.sh-906-67" />
                    </div>
                    <div class="register-avatar" style="background-image: url(/habbo-imaging/avatar/hr-170-44.hd-207-9.ch-210-84.lg-285-75.sh-290-67,s-0.g-1.d-4.h-4.a-0,430994a364251faa1302b37ca50f6ea9.gif)">
                        <input type="radio" name="randomFigure" value="M-hr-170-44.hd-207-9.ch-210-84.lg-285-75.sh-290-67" />
                    </div>
                    <div class="register-avatar" style="background-image: url(/habbo-imaging/avatar/hr-145-33.hd-180-10.ch-250-91.lg-270-64.sh-290-75.ea-1406-,s-0.g-1.d-4.h-4.a-0,c52a0478283776654483107d0ca2a2fd.gif)">
                        <input type="radio" name="randomFigure" value="M-hr-145-33.hd-180-10.ch-250-91.lg-270-64.sh-290-75.ea-1406-" />
                    </div>
                </div>
                <p><input type="submit" name="refresh" value="Mostrar m&aacute;s <?php echo $shortname; ?>s" id="register-avatars-refresh" /></p>
            </div>

            <!-- Flash look editor via Ruffle (WASM), isolated in an iframe -->
            <script type="text/javascript" language="JavaScript">
(function(){
	var c = document.getElementById("register-avatar-editor");
	if(c){
		c.innerHTML = '<iframe src="flash_editor.php?figure=<?php echo isset($mylook1) ? urlencode($mylook1) : ""; ?>&gender=<?php echo (isset($mysex1) && $mysex1) ? $mysex1 : "M"; ?>&hc=0" width="435" height="400" frameborder="0" scrolling="no" style="border:0;overflow:hidden;"></iframe>';
	}
	window.addEventListener("message", function(ev){
		if(ev.data && ev.data.type === "habboFigure" && ev.data.figure){
			var fig = document.getElementById("register-figure"); if(fig){ fig.value = ev.data.figure; }
			if(ev.data.gender){ var g = document.getElementById("register-gender"); if(g){ g.value = ev.data.gender; } }
		}
	}, false);
})();
</script>
            ...
            <div id="register-section-2">
                <div class="rounded rounded-blue">
                    ...
                    <fieldset id="register-fieldset-name">
                        <div class="register-label white">Tu nombre de usuario en <?php echo $shortname; ?></div>
            ...
            <?php } ?>
                ...
                <?php } ?>
                        ...
                        <fieldset>
                            ...
                            <div class="register-label"><label for="register-email2">Conf&iacute;rmalo:</label></div>
                        ...
                        <div id="register-marketing-box">
                            ...
                            <label for="bean_marketing">S&iacute;, quiero recibir informaci&oacute;n de <?php echo $shortname; ?> (Newsletters)</label>
                        </div>                  
                        ...
                        <fieldset id="register-fieldset-terms">
                            <div class="rounded rounded-darkgray" id="register-terms">
                                <div id="register-terms-content">
                                    <p><a href="./papers/disclaimer.php" target="_blank" id="register-terms-link">Condiciones de uso</a></p>
                                    <p class="last">
                                        ...
                                        <label for="register-terms-check">Acepto las condiciones.</label>
											<p>Soy <select name="beanBot">
											<option value="no">...</option>
											<option value="yes">un humano</option>
											<option value="no">un bot</option>
										</select>
                                </div>
                            </div>
                        </fieldset>
                ...
                <div id="form-validation-error-box" style="display:none">
                    <div class="register-error">
                        <div class="rounded rounded-red">
                            El registro no pudo completarse, corrige los campos en rojo.
                        </div>
                    </div>
                </div>
            </div>
