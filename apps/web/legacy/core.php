...
if(!session_is_registered(username) && $_COOKIE['remember'] == "remember"){
	...
	$cname = FilterText($_COOKIE['srausername']);
	$cpass_hash = $_COOKIE['srapassword'];
...
function HCDaysLeft($my_id){
	...
	if($valid > 0){
		...
		$then = mktime(0, 0, 0, $month, $day, $year); // 7th arg (is_dst) removed in PHP 7.0
		$now = time();
		$difference = $now - $then;
