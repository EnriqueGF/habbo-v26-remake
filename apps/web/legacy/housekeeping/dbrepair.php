...
$msg = "Reparando tablas..</strong> (Nota: Las tablas pueden no estar 'rotas', pero esta funci&oacute;n <i>intenta</i> repararlas independientemente de si es necesario)";
$getTables = mysql_query("SHOW TABLES FROM " . $sqldb) or die(mysql_error());
    while($tmp = mysql_fetch_array($getTables)){
        $table = $tmp[0];
        $msg = $msg . "<br />Reparando tabla " . $tmp[0] . "..";
        mysql_query("REPAIR TABLE " . $tmp[0]) or die(mysql_error());
        $msg = $msg . "<strong>OK!</strong>";
    }
$msg = $msg . "<br /><br />&iexcl;Tablas de la base de datos reparadas correctamente!<strong>";
