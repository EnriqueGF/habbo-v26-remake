...
<?php @include('usermenu.php'); ?>
 ...
 <div class='tableheaderalt'><?php echo $sitename; ?> - Lista de usuarios</div>
...
while($row = mysql_fetch_assoc($get_users)){
        ...
        if($clock == time()){ // value was null or zero, means there was no activity logged for this user .. ever
                ...
                if($clock > 24){
                        $clock = ceil($clock / 24) . " d&iacute;a(s)";
                } else {
                        $clock = $clock . " hora(s)";
                }
        } elseif($clock > 600){
                $clock = ceil($clock / 60) . " minuto(s)";
        } elseif($clock < 600){
                $clock = ceil($clock) . " segundo(s)";
        } else {
                $clock = "Fuera de rango";
        }
	
	if(empty($row['ipaddress_last'])){ $row['ipaddress_last'] = "IP no encontrada"; }
	printf(" <tr>
