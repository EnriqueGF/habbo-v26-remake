...
if (is_writable($filename)) {
    if (!$handle = fopen($filename, 'a')) {
         $msg =  "No se puede abrir el fichero ($filename)";
         exit;
    }
    if (fwrite($handle, $somecontent) === FALSE) {
        $msg = "No se puede a&ntilde;adir el sticker";
        exit;
    }
    $msg = "El sticker/fondo se ha a&ntilde;adido correctamente";

    fclose($handle);

} else {
    $msg = "El fichero $filename no tiene permisos de escritura";
}
			...
			mysql_query("INSERT INTO system_stafflog (action,message,note,userid,targetid,timestamp) VALUES ('Housekeeping','Created Sticker or Background ".FilterText($_POST['$name'])." worth ".$price." credits','add_homes_stuff.php','".$my_id."','','".$date_full."')") or die(mysql_error());

        }
    } else {

        $msg = "No dejes ning&uacute;n campo en blanco.";
...
<td class='tablerow1'  width='40%'  valign='middle'><strong>Category</strong><div class='graytext'>
    La categor&iacute;a del elemento</div></td>
...
<td class='tablerow1'  width='40%'  valign='middle'><strong>Name</strong><div class='graytext'>
    El nombre que se mostrar&aacute; con la imagen en el cat&aacute;logo de hogares y grupos</div></td>
<td class='tablerow2'  width='60%'  valign='middle'>
...
<td class='tablerow1'  width='40%'  valign='middle'><strong>Price</strong><div class='graytext'>
    La cantidad de cr&eacute;ditos que costar&aacute; el elemento</div></td>
<td class='tablerow2'  width='60%'  valign='middle'>
<input type='text' name='price' value="" size='30' class='textinput'></td>
</tr>

<tr><td align='center' class='tablesubheader' colspan='2' ><input type='submit' value='Crear sticker' class='realbutton' accesskey='s'></td></tr>
</form></table></div><br />
</div><!-- / RIGHT CONTENT BLOCK -->
     </td></tr>
</table>
