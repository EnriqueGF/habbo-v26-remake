...
if(!isset($_POST['category']) || !isset($_GET['category'])){ // do not try to save when it's a category jump
    foreach($_POST as $key => $value){
        if(strlen($value) > 0 && strlen($key) > 0){
            ...
            $msg = "Ajustes guardados correctamente.<br />"; // I know, we shouldn't do this in a loop but meh, doesn't really matter that much, does it?
        }
    }
}
...
<tr>
    <td class='tablerow2'  width='100%'  valign='middle'  align='center'>
        <select name='category' class='dropdown'>
            <option value='1' <?php if($catId == "1"){ echo "selected='selected'"; } ?>>Opciones generales</option>
            <option value='2' <?php if($catId == "2"){ echo "selected='selected'"; } ?>>M&oacute;dulo de personal</option>
            <option value='3' <?php if($catId == "3"){ echo "selected='selected'"; } ?>>P&aacute;gina de cr&eacute;ditos</option>
            <option value='4' <?php if($catId == "4"){ echo "selected='selected'"; } ?>>M&oacute;dulo de foro</option>
            <option value='5' <?php if($catId == "5"){ echo "selected='selected'"; } ?>>Bolet&iacute;n</option>
        </select>
        &nbsp;
...
<div class='tableborder'>
<div class='tableheaderalt'>Gestionar opciones de contenido</div>
...
<td class='tablerow2'  width='60%'  valign='middle'>";
    if($row['fieldtype'] == "1"){
        ...
        echo "<select name='".$row['contentkey']."' class='dropdown'><option value='1'>Activado</option><option value='0'"; if($row['contentvalue'] == "0"){ echo " selected='selected'"; } echo ">Desactivado</option></select>";
    } elseif($row['fieldtype'] == "3"){
        echo "<select name='".$row['contentkey']."' class='dropdown'><option value='red'"; if($row['contentvalue'] == "red"){ echo " selected='selected'"; } echo ">Rojo</option><option value='blue'"; if($row['contentvalue'] == "blue"){ echo " selected='selected'"; } echo ">Azul</option><option value='white'"; if($row['contentvalue'] == "white"){ echo " selected='selected'"; } echo ">Blanco (sin cabecera)</option><option value='green'"; if($row['contentvalue'] == "green"){ echo " selected='selected'"; } echo ">Verde</option></select>";
    }
    echo "</td>
</tr>";
    }
...
<tr>
<tr><td align='center' class='tablesubheader' colspan='2' ><input type='submit' value='Guardar ajustes' class='realbutton' accesskey='s'></td></tr>
