...
namespace Holo.Managers
{
    ...
    public static class userManager
    {
        ...
        public static void addUser(int userID, virtualUser User)
        {
            ...
            // IP check disabled for the dockerized / ws<->tcp-proxied setup: the
            // emulator sees the proxy container's IP, never the browser's, so
            // ipaddress_last would never match. The SSO ticket lookup that led
            // here already authenticated the user.
            if (true || User.connectionRemoteIP == DB.runRead("SELECT ipaddress_last FROM users WHERE name = '" + User._Username + "'"))
            {
